...
exports.addItem = async (req, res) => {
  try {
    ...
    const [result] = await pool.query('INSERT INTO items (description, completed, list_id) VALUES (?, ?, ?)', [description, 0, list_id]);
...
exports.updateItem = async (req, res) => {
  try {
    ...
    const [result] = await pool.query('UPDATE items SET completed = ? WHERE id = ?', [completed, id]);
